...
public class RegistrarQuarto extends JInternalFrame {
	...
	public void inserirDados() {
		...
		if (qrt.getIdQuarto().isStatusQuarto() == false) {
			if (qrt.getIdHospede().isStatusHospede() == true) {
				DaoSupplier.getDaoQuarto().update(qrt.getIdQuarto());
				DaoSupplier.getDaoRegistrarQuarto().insert(qrt);
				sair();
				JOptionPane.showMessageDialog(null,
						"Quarto registrado para o usuário: " + qrt.getIdHospede().getNome());
			} else if(qrt.getIdHospede().isStatusHospede() == false) {
		
				JOptionPane.showMessageDialog(null, "Hospede inativo");
			
			}
		
		}
		else if (qrt.getIdQuarto().isStatusQuarto() == true || qrt.getIdHospede().isStatusHospede() == false) {
